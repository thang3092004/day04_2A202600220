...
def search_flights(origin: str, destination: str) -> str:
    ...
    for idx, f in enumerate(flights, 1):
        ...
        result += f"{idx}. {f['airline']} | {f['departure']} - {f['arrival']} | Giá: {price_str}đ\n"
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    if not filtered_hotels:
        price_str = f"{max_price_per_night:,}".replace(",", ".")
        return f"Không tìm thấy khách sạn tại {city} với giá dưới {price_str}đ/đêm. Hãy thử tăng ngân sách."
        
    result = f"Các khách sạn tại {city} phù hợp:\n"
    for idx, h in enumerate(filtered_hotels, 1):
        price_str = f"{h['price_per_night']:,}".replace(",", ".")
        result += f"{idx}. {h['name']} ({h['stars']} sao) | Khu vực: {h['area']} | Đánh giá: {h['rating']} | Giá: {price_str}đ/đêm\n"
    ...
    try:
        ...
        if expenses.strip():
            ...
            for item in items:
                ...
                expense_dict[name.strip().replace('_', ' ').capitalize()] = int(amount.strip())
        ...
        for name, amount in expense_dict.items():
            result += f"- {name}: {f'{amount:,}'.replace(',', '.')}đ\n"
            
        result += "---\n"
        result += f"Tổng chi: {f'{total_expense:,}'.replace(',', '.')}đ\n"
        result += f"Ngân sách: {f'{total_budget:,}'.replace(',', '.')}đ\n"
        result += f"Còn lại: {f'{remaining:,}'.replace(',', '.')}đ\n"
        
        if remaining < 0:
            result += f"\nVượt ngân sách {f'{-remaining:,}'.replace(',', '.')} đồng! Cần điều chỉnh."
            
        return result
    except Exception as e:
        return f"Lỗi nội dung 'expenses'. Vui lòng dùng đúng định dạng 'tên_khoản:số_tiền,tên_khoản:số_tiền'. (Chi tiết lỗi: {e})"